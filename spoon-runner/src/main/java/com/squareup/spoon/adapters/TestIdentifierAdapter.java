...
public enum TestIdentifierAdapter {
    ...
    CUCUMBER() {
        @Override
        public TestIdentifier adapt(TestIdentifier test) {
            String className = WordUtils.capitalize(test.getClassName()).replaceAll("[^a-zA-Z0-9_]+", "");
            String testName = "test" + WordUtils.capitalize(test.getTestName()).replaceAll("[^a-zA-Z0-9_]+", "");
