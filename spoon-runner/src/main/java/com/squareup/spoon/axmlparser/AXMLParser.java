/*
 * Copyright 2008 Android4ME Licensed under the Apache License, Version 2.0 (the "License"); you may
 ...
 * 
 *         TODO: * understand ? values
 */
public final class AXMLParser {

    /**
     * Types of returned tags. Values are compatible to those in XmlPullParser.
     */
    public static final int START_DOCUMENT = 0, END_DOCUMENT = 1, START_TAG = 2, END_TAG = 3, TEXT = 4;

    /**
     * Creates object and reads file info. Call next() to read first tag.
     */
    ...
    /**
     * Closes parser: * closes (and nulls) underlying stream * nulls dynamic data * moves object to
     * 'closed' state, where methods return invalid values and next() throws IOException.
     */
    ...
    /**
     * Advances to the next tag. Once method returns END_DOCUMENT, it always returns END_DOCUMENT.
     * Once method throws an exception, it always throws the same exception.
     * 
     */
    public final int next() throws IOException {
        if (m_nextException != null) { throw m_nextException; }
        try {
            return doNext();
        } catch (IOException e) {
            m_nextException = e;
    ...
    public final int getAttributeCount() {
        if (m_tagAttributes == null) { return -1; }
    ...
    public final int getAttributeResourceID(int index) {
        ...
        if (m_resourceIDs == null || resourceIndex < 0 || resourceIndex >= m_resourceIDs.length) { return 0; }
        return m_resourceIDs[resourceIndex];
    }

    /**
     * Returns type of attribute value. See TypedValue.TYPE_ values.
     */
    ...
    /**
     * For attributes of type TypedValue.TYPE_STRING returns string value. For other types returns
     * empty string.
     */
    ...
    /**
     * Returns integer attribute value. This integer interpreted according to attribute type.
     */
    ...
    private final void doStart() throws IOException {
        ...
        if (chunkSize < 8 || (chunkSize % 4) != 0) { throw new IOException("Invalid resource ids size (" + chunkSize + ")."); }
    ...
    private final int doNext() throws IOException {
        if (m_tagType == END_DOCUMENT) { return END_DOCUMENT; }
    ...
    private final TagAttribute getAttribute(int index) {
        if (m_tagAttributes == null) { throw new IndexOutOfBoundsException("Attributes are not available."); }
        if (index >= m_tagAttributes.length) { throw new IndexOutOfBoundsException("Invalid attribute index (" + index + ")."); }
    ...
    private static final int AXML_CHUNK_TYPE = 0x00080003, RESOURCEIDS_CHUNK_TYPE = 0x00080180;
}
