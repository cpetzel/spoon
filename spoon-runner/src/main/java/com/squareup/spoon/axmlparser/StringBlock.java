...
public class StringBlock {
    ...
    public static StringBlock read(IntReader reader) throws IOException {
        ...
        }
        {
            ...
            if ((size % 4) != 0) { throw new IOException("String data size is not multiple of 4 (" + size + ")."); }
            block.m_strings = reader.readIntArray(size / 4);
        }
        if (stylesOffset != 0) {
            int size = (chunkSize - stylesOffset);
            if ((size % 4) != 0) { throw new IOException("Style data size is not multiple of 4 (" + size + ")."); }
    ...
    /**
     * Returns raw string (without any styling information) at specified index. Returns null if
     * index is invalid or object was not initialized.
     */
    public String getRaw(int index) {
        if (index < 0 || m_stringOffsets == null || index >= m_stringOffsets.length) { return null; }
     ...
     * Not yet implemented.
     * 
     * Returns string with style information (if any). Returns null if index is invalid or object
     * was not initialized.
     */
    ...
    private int[] getStyle(int index) {
        if (m_styleOffsets == null || m_styles == null || index >= m_styleOffsets.length) { return null; }
